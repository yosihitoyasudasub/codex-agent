...
> **未選択**。プロジェクト開始時に以下を実行してスタックを選択してください:
> `.codex/workflows/select-stack.md` を参照
>
> 選択後、このセクションはスタック固有の情報（技術スタック・ビルドコマンド・Dockerコマンド）に更新されます。
...
### テストピラミッド
- Unit Test（多数・高速）: ビジネスロジック
- Integration Test（中程度）: API・DB層
...
- スタック別のテスト配置は `docs/development-guidelines.md` を参照
